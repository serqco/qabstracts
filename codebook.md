...
changed: Lutz Prechelt, 2022-10-03
...
- Exception: When there is no blank after a sentence end, our script will
...
- Exception: Codes starting with a dash ("minor codes"), such as `-hype`, 
  may be present in one coding without being present in the other.
- Exception 2: If a code is marked as unsure, `somecode?`,
...
- Verbs are often uninteresting glue, so they do not count, even if they
  _happen to be_ informative in the particular case.
  Ditto for past participles and present participles acting in a verb-like role.
  This rule helps identify sentences of low informativeness.
- Counting is based on noun phrases, each having 1 (the default) or 0 or several information particles.
- Some phrases are almost never informative and hence count as 0, 
  e.g. "this paper", "a study".
  (A phrase that only _happens to be_ uninformative is not ignored.)
- A basic noun phrase (counting 1) consists of a noun (perhaps compound or proper) 
  and 0 or 1 adjectives.
- Each additional adjective counts 1. Each number (with its unit of measurement) counts 1.
- Articles, prepositions, conjunctions count 0.
- Additional components of the noun phrase such as
  prepositional phrases, infinitive phrases, relative clauses, and others
  are considered separately (which sometimes results in ignoring them).  
...
- <i>"We evaluated the attack effectiveness using 5 state-of-the-art deep learning models and  
  ...
  - 0: "We": pronoun
  - 0: "evaluated": verb
  - 1: "the attack effectiveness": compound noun
  - 0: "using": verb (not really; we are simplifying some grammar concepts)
  - 2: "5 state-of-the-art deep learning models": number, compound noun with adjective
  - 1: "real-world samples": noun with adjective
  - 0: "collected": verb
  - 2: "from 30 users": number, noun
- <i>"BeAFix is backed with a novel strategy for bounded exhaustive, yet scalable, exploration of 
  ...
  - 1: "novel strategy": basic noun phrase
  ...
  - 1: "formally rigorous": adjective (indeed an adjective phrase with an adverb; we are simplifying again)
...
- <i>"To overcome these challenges and aid developers in this task, this paper presents TANGO, a duplicate
  ...
  textual information."</i> `{{objective,design:5}}`
  ...
  - 1: "this paper presents TANGO": ("this paper" ignored), (verb ignored), proper noun
  - 1: "duplicate detection technique": proper noun (there is no adjective involved!)
  - 1: "operates purely on video-based bug reports": (verb phrase "operates purely" ignored), compound noun with adjective
  - 2: "visual and textual information": adjective, noun with adjective
- <i>"We describe the design of a Crowd Planning Poker (CPP) process implemented on Amazon Mechanical Turk 
  ...
  - Method part:
    - 3: "results of a substantial set of trials": noun, noun with adjective, noun.
      ...
      in the present case (but not all others) it makes no difference for the count.)
