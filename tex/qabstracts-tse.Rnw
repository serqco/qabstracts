...
% Alternative article references: Link to raw abstract on GitHub
\renewcommand{\Art}[1]{\href{https://github.com/serqco/qabstracts/blob/main/abstracts/abstracts.raw/\string#1.txt}{\bgroup[#1]\egroup}}
...
\newcommand{\Quote}[1]{\bgroup\itshape\color{purple} ``#1''\egroup}  % quote from a specific real abstract
\newcommand{\ExampleQuote}[1]{\bgroup\itshape ``#1''\egroup}         % generic quote from real abstract
\newcommand{\Pseudoquote}[1]{\bgroup\itshape\color{ForestGreen} `#1'\egroup}            % quote from fictive abstract
\newcommand{\QuoteCB}[1]{\bgroup\itshape\color{ProcessBlue} ``#1''\egroup}  % quote from codebook
...
Julian Frattini%
\thanks{J. Frattini is with Chalmers University of Technology, Gothenberg, Sweden},
Franz Zieris%
...
% Note: Make sure to update `our-abstract.txt'.
%       That's the basis for calculating our own FK score.
...
\Todo{For Lutz: Where would be a good place for \cite{ShiGalMil24},
survey among ``science and health academics and researchers'', n=139:
``98.6\% of the sample read the abstract first because it provides an
overview of the paper and facilitates a decision on continuing to read on or not''}
...
\begin{description}
	...
	\item[RQ2] We describe and discuss nine types of deficiencies,
...
\Todo{For Lutz: That Section does not make it sound ``very different'' or ``more complicated''. What did you have in mind?}.
...
but note that both coding and statistical evaluation
are compatible with a \emph{positivist} epistemology (aiming for ``objective'' results),
...
\Cb{summary} summarizes several results, but do not provide new information;
\Cb{conclusion} generalizes beyond the immediate results.
\Cb{fposs}, \Cb{fneed}, and \Cb{fwork} sentences occur at the end of abstracts;
...
but the authors could and should have given that information in the abstract.
...
because it is unclear from the abstract what the \Quote{UNK problems} are.
...
\textbf{Step~2.} He then ran the \Prg{check-codings} script to test his codings
...
\begin{lstlisting}[language={},
  ...
  basicstyle=\scriptsize\color{purple}]
...
results <- abstract |>
  ...
  mutate(
    ...
    syllable_count = map_int(word_list, ~sum(map_int(.x, syllablecount)))
  )
...
and the \Cb{design} code will \emph{not} be used.
Artifact design discussions were then usually coded as \Cb{method} instead.
Mixed cases are rare and there the two coders decided which aspect has more weight.
...
\emph{``Engineering Web Augmentation software: A development method for enabling end-user maintenance''} \Art{FirFirRos22}
(all \Cb{background} plus \Cb{objective} in boldface):
\begin{quote}
  \Todo{All examples with high background ratio are long. Move to appendix?}
  Nowadays, end-users are able to adapt Web applications when some of their requirements have not been taken into account by developers.
  One possible way to do adaptations is by using Web Augmentation techniques.
  Web Augmentation allows end-users to modify the Web sites' user interfaces once these are loaded on the clientside, i.e., in the browser.
  ...
  This particular kind of software artifacts requires special attention regarding maintenance as---in most cases---they depend on third-party resources, such
  as HTML pages.
  When these resources are upgraded, unexpected results during the augmentation process may occur.
  Many communities have arisen around Web Augmentation, and today there are large repositories where developers share their augmenters; end-users may give feedback about existing augmentations and even ask for new ones.
  Maintenance is a key phase in the augmenters' life-cycle, and currently, this task falls (as usual) on the developers.
  \textbf{In this paper, we present a participatory approach for allowing end-users without programming skills to participate in the augmenters' maintenance phase.}
\end{quote}
<<>>=
...
long_bg_example <- df_c |>
  ...
  rename(words = words_backgroundcode, ratio = fraction_code_background)
@
That's \Sexpr{long_bg_example$words} words spent on \Cb{background}
(\Sexpr{format_percent(long_bg_example$ratio/100)} of the abstract)
before the \Cb{objective} is formulated.

<<>>=
# Fetch concrete numbers for example abstract
short_bg_example <- df_c |>
  filter(`_citekey` == 'LavMor22') |>
  slice_head(n=1) |>
  select(words_backgroundcode, fraction_code_background) |>
  rename(words = words_backgroundcode, ratio = fraction_code_background)
@
Other abstracts manage to solve the same problem in a wonderfully concise manner,
e.g., \Art{LavMor22} (\Sexpr{format_percent(short_bg_example$ratio/100)} background):
\begin{quote}
	The F-measure has been widely used as a performance metric when selecting
binary classifiers for prediction, but it has also been widely criticized, especially given the
availability of alternatives such as $\varphi$ (also known as Matthews Correlation Coefficient).
  \textbf{Our goals are to (1) investigate possible issues related to the F-measure in depth and show how $\varphi$ can address them, and (2) explore the relationships between the F-measure and $\varphi$.}
\end{quote}	
...
\Todo{Franz looked for such examples. Only idea: look for coded suffixes. But: None of those 15 statements is clearly "murky".
Proposal: Remove section since the criterion is not operationalized and quantified.}
...
\Todo{For Lutz: Huge difference between `fraction\_code\_conlusion' (for the conclusion code only, missing in 62\%, used in text),
and `fraction\_conclusion' (includes h-conclusion and a-conclusion, missing in 52\%, used in plot). Which do we take?}
...
but they are always present in structured abstracts.
...
half of all (\Sexpr{format_percent(r_no_conclusion$r_missing)}).
...
\footnote{Note that
  ...
  does not guarantee an actual \Cb{conclusion} follows:
...
For quantifying this, we map each abstract's structure to a sequence of letters,
...
(Fractional frequencies such as $7.5$ mean that the two coders did not agree.)
...
cumulative_ratio <- function(design, n) {
  x <- df |>
    select(`_citekey`, is_design) |>
    distinct() |>
    left_join(datasets$ab_structures, c("_citekey" = "citekey")) |>
...
We see that the most sensible structures are also the most frequent
(bars 1, 2, 3; accounting for \Sexpr{format_percent(cumulative_ratio(FALSE, 3))} of the empirical articles):
...
They are missing a \Cb{conclusion}, discuss their \Cb{method} before the \Cb{objective},
and later later on (not shown in the figure)
follow all kinds of curious structures such as, to pick an admittedly extreme case,
\strct{bgomrcrsrsc} (e.g., \Art{\Sexpr{strc_example("bgomrcrsrsc")}}):
two conclusions, two summaries, summary after conclusion.

\Todo{The example TyaSibSur22 is only "weird" because in the sense that it crams results and summaries under its "Conclusion" heading -- which is also the first of the "two" conclusions. Maybe h- codes should be excluded from the structure string?}
...
The two most common structures (accounting for \Sexpr{format_percent(cumulative_ratio(TRUE, 2))} of the design articles)
are missing a \Cb{conclusion},
and we find complex structures with repeated \strct{mr}'s starting at bar 4.
Not all of these are automatically bad.
In particular, having two small empirical studies, resulting in a nice \strct{bgodmrmr} structure
(e.g., \Art{WuZouDou22}), can make a lot of sense for the reader.
...
\strct{bodrmrmrmrmrmr} (e.g. \Art{PisSpäBud22}) and
\strct{bomdmrmrmrsmrc} (e.g. \Art{ValManPar22}),
which lack a coherent \emph{Method} part.
...
design_empir_struct <- ab_is_design |> filter(!is_design, is_struc)
design_empir_all    <- ab_is_design |> filter(!is_design)

# Empirical and structured: Just count
(n_distinct_empir_struct <- design_empir_struct |>
  summarize(count = n_distinct(topicstructure)) |>
  pull(count))

# All empirical: Take same-size sample of abstract and count there
c_design_empir_struct <- nrow(design_empir_struct)

set.seed(123)
resample_counts <- replicate(
  1000,
  {
    sample_n(design_empir_all, c_design_empir_struct,
        replace = FALSE) |>
...
(n_distinct_empir_all <- median(resample_counts))
(n_distinct_empir_all_ci <- quantile(resample_counts, c(0.025, 0.975)))
@
...
versus \Sexpr{round(n_distinct_empir_all)}
across all empirical articles.\footnote{%
  Median of 1,000 same-sized samples; 95\%~CI
[\Sexpr{round(n_distinct_empir_all_ci[1])}, \Sexpr{round(n_distinct_empir_all_ci[2])}].
}
...
This is deplorable, because for the vast majority readers of a typical article,
the abstract is the first part they read \cite{ShiGalMil24} --- and often the only.
Futhermore, indexes and search engines such as \emph{Scopus AI}\footnote{%
...
\Todo{now holds a PhD degree and is no longer}
