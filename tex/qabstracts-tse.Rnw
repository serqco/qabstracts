...
\definecolor{codebkecolor}{RGB}{0, 165, 218}
...
\begin{quote}
  ...
  \color{abstractcolor}\sffamily\small
...
\footnote{Note that
  the prompt of having to write something after a \ExampleQuote{Conclusion:} keyword
  does not guarantee an actual \Cb{conclusion} follows:
  Often, the statement authors put there is actually
  a \Cb{result} (an immediate empirical result)
  or a \Cb{summary} (a mere repetition).}
...
that have the same topic (see Table~\ref{tab:codes}).
