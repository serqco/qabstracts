...
common (20-35\%),
frequent (35-50\%),
dominant (over 50\%).  \Todo{Are 'frequent' and 'dominant' used at least once? The others are.} 
...
the following template, which we call the \emph{SERQco software engineering abstracts archetype}
...
all venues that do not require structured abstract, have a quarter of articles or more 
that spend over 30% of the abstract space
on background. 
...
\footnote{The prompt of having to write something after a \ExampleQuote{Conclusion:} keyword
  ...
  a \Cb{summary} (a mere repetition) or still something else.}
...
There are also abstracts where the train-of-thought is so tangled
...
we restrict ourselves to the types that occur with higher frequency here\footnote{One reason
  for the large number of types is the frequent violation of \Cb{h-xyz} codes:
  If \Cb{h-xyz} is not followed by \Cb{xyz}, this will lead to two letters in the string.}
...
and later on (not shown in the figure)
follow all kinds of curious structures such as, to pick an admittedly extreme case,
\strct{bgomrcrsrsc} \Art{\Sexpr{strc_example("bgomrcrsrsc")}}:
...
obtained his Ph.D. degree from the Blekinge Institute of Technology, Sweden, and is currently a postdoctoral researcher at the Chalmers University of Technology and University of Gothenburg, Sweden.
...
Julian served in the organizing committees of REFSQ'24, RE'24, AIRE'24, CrowdRE'24, REFSQ'25, and AIRE'25, and is the publicity co-chair of the IST journal.
He is on the program committees of RE, REFSQ, EASE, ESEM, QUATIC, and NLP4RE, and regularly reviews for the journals TSE, TOSEM, EMSE, REJ, IST, and JSS.
\end{IEEEbiography}
