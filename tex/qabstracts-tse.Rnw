...
only about \Sexpr{r_read_good} of all abstracts have good readability,
about \Sexpr{r_read_normal} have normal readability,
and \Sexpr{r_read_improper} are improper ---
several (\Sexpr{r_read_lt10}) are even below 10!
...
stats_overview |>
  ...
  mutate(
    name  = stringr::str_extract(name, "[^_]+$"),
  ...
  pack_rows("Readability (Section \\ref{sec:result-len-read})", 1, 3, escape = FALSE) |>
  pack_rows("Completeness (Section \\ref{missing_elements})", 4, 5, escape = FALSE) |>
  pack_rows("Informativeness Gap (Section \\ref{igaps})", 6, 7, escape = FALSE) |>
  pack_rows("Announcements (Section \\ref{announcements})", 8, 9, escape = FALSE) |>
  pack_rows("Understandability Gap (Section \\ref{ugaps})", 10, 11, escape = FALSE) |>
  pack_rows("Ambiguous Formulations (Section \\ref{ambiguity})", 12, 13, escape = FALSE) |>
  pack_rows("Properness (Section \\ref{hownotto_summary})", 14, 15, escape = FALSE) |>
  row_spec(c(3, 5, 7, 9, 11, 13, 15), italic = TRUE)
@
