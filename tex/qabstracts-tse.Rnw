...
(r_read_good <- sum(df$fkscore > 30) / nrow(df))
...
\emph{Results:}
Only \Sexpr{format_percent(r_complete)} of the sampled abstracts are \textit{complete},
...
Only \Sexpr{format_percent(r_proper)} of the abstracts are \textit{proper},
...
The present article aims to substantiate this impression
by operationalizing \emph{abstracts quality} and analyzing hundreds of abstracts,
and formulates constructive advice for writing better abstracts.
...
No firm expectations regarding RQ1 and RQ2 exist for engineering articles,
...
regarding our answers to RQ2, RQ3, and RQ4, and regarding the need to improve
the quality of software engineering abstracts.
Therefore, we continue the research by performing a repeatable
quantitative content analysis~\cite[Ch. 7]{Krippendorff04} for RQ2
...
Genres are text types \Todo{Such as?};
...
df |>
  reframe(qs = quantile(words, bp_prob), prob = bp_prob * 100,
    .by = is_struc) |>
  pivot_wider(id_cols = is_struc, values_from = qs,
...
df |>
  mutate(max_words = case_match(venue,
    c("EMSE", "TSE") ~ 250, "IST" ~ 300)) |>
  summarize(r_over = sum(words > max_words) / n(), .by = venue)
@
...
<<>>=
# Readability scores
(r_read_normal <- sum(between(df$fkscore, 20, 30)) / nrow(df))
(r_read_improper <- sum(df$fkscore < 20) / nrow(df))
(r_read_lt10 <- sum(df$fkscore < 10) / nrow(df))
@

<<>>=
# Readability by venue
(venue_mean_fkscores <- df |>
  summarize(m = mean(fkscore), mf = round(m), .by = venue))
(venue_best  <- venue_mean_fkscores |> slice_max(m, with_ties = FALSE))
(venue_worst <- venue_mean_fkscores |> slice_min(m, with_ties = FALSE))
@

For readability, see Figure~\ref{boxplots_fkscore}:
only about \Sexpr{format_percent(r_read_good)} of all abstracts have good readability,
about \Sexpr{format_percent(r_read_normal)} have normal readability,
and \Sexpr{format_percent(r_read_improper)} are improper ---
several (\Sexpr{format_percent(r_read_lt10)}) are even below 10!
\Sexpr{venue_best$venue} tends to be better than the other venues,
\Sexpr{venue_worst$venue} is worse.
...
\Plot{boxplots_fkscore}{%
    ...
    \Sexpr{venue_best$venue} is best (mean \Sexpr{venue_best$mf}),
    \Sexpr{venue_worst$venue} is worst (mean \Sexpr{venue_worst$mf}).
    ...
    The abstract of the present article has a score of 24.5.
    % https://oscarstories.com/calculator/flesch-reading-ease/
}
...
# Typical fractions for design works nd empirical works
typl_frac <- df_frac |>
  summarize(
...
df |>
  reframe(qs = quantile(words, bp_prob), prob = bp_prob * 100,
    .by = is_design) |>
  pivot_wider(id_cols = is_design, values_from = qs,
    names_from = prob, names_prefix = "p")
@
...
# Missing parts
df_frac |>
  summarize(r_missing = sum(value == 0) / n(),
    .by = c(part, is_struc)) |>
...
assert_exists <- function(x, what) {
  if (length(x) == 0) paste0("\\Todo{", what, " does not exist}") else x
}
