...
knitr::opts_chunk$set(
    echo = FALSE,    # change to TRUE to see all R code
    results = 'hide' # change to 'markup' to see intermediate values
) 
@
...
format_percent <- function(value, accuracy = 1) {
  scales::label_percent(accuracy)(value) |> as.character() |> stringr::str_replace("%", "\\\\%")
}
@

<<>>=
(n_abstracts <- n_distinct(df$`_citekey`))
(r_complete <- sum(df$is_complete) / nrow(df))
(r_readable_good <- sum(df$fkscore > 30) / nrow(df))
(r_readable_normal <- sum(between(df$fkscore, 20, 30)) / nrow(df))
(r_readable_improper <- sum(df$fkscore < 20) / nrow(df))
@
...
over a sample of \Sexpr{n_abstracts} abstracts from five presumably high-quality venues.
...
\emph{Results:}
Only \Sexpr{format_percent(r_complete)} of the sampled abstracts are complete, i.e., provide background, objective,
method, result, and conclusion information.
For structured abstracts, the fraction is twice as big.
Only \Sexpr{format_percent(r_readable_good)} of the abstracts also have good readability (Flesch-Kincaide score) and have
neither informativeness gaps nor understandability gaps nor highly ambiguous sentences.
\emph{Conclusions:}
...
We start our research by open coding~\cite[Ch. 5]{StrCor90}
in order to derive a vocabulary (a set of concepts or codes)
...
regarding our answers to questions 2, 3 and 4, and regarding the need to improve
the quality of software engineering abstracts.
Therefore, we continue the research by performing a repeatable
quantitative content analysis~\cite[Ch. 7]{Krippendorff04} for number 2
...
the questions it asks and the applied statistical methods.
...
Swales~\cite{Swales90} introduced ``genre analysis'' as a means for teaching academic
...
the answer is: structured abstracts have fewer quality issues.\footnote{A structured abstract is one that uses a prescribed sequence of 
...
When we refer to abstracts from our sample, we use abbreviations of the first three authors' names and the year of publication, e.g. [BesMarBos22].
...
This sentence was coded as \Cb{design:u1} because it is unclear from the abstract what the unintroduced concept of \Quote{UNK problems} are, and furthermore, what that means for \Quote{code summaries}.
...
% \indent -> I don't think this is doing anything. Maybe a style issue?
...
\Todo{For Lutz: What about Step 5a? Lloyd: I don't know what 5a is.}
\textbf{Step~6.} Lutz would read through that email and categorize the report blocks into the following cases:
\begin{enumerate}[label=\alph*)]
  \item Lloyd's coding is obviously correct (Lutz' own is a clerical error) or Lutz prefers Lloyds coding.
  Lutz adjusts his coding accordingly.
  \item Lutz finds Lloyd's coding clearly incorrect (clerical error) or Lutz prefers his own coding.
  He would respond with an explanation of his reasoning and suggest that Lloyd either adjust his coding or add an \Cb{-ignorediff} marker to it.
  This suffix indicates two accepted alternative interpretations of the same sentence.
  \footnote{It also silences the \Prg{compare-codings} script for this particular report block, so that the coding difference is now officially accepted.}
...
<<>>=
(r_readable_lt10 <- sum(df$fkscore < 10) / nrow(df))
@

\Todo{Figure~\ref{boxplots_fkscore} double-counts abstracts (blocks A and B)}
For readability, see Figure~\ref{boxplots_fkscore}:
only about \Sexpr{format_percent(r_readable_good)} of all abstracts have good readability,
about \Sexpr{format_percent(r_readable_normal)} have normal readability,
and a majority is improper --- almost a quarter (\Sexpr{format_percent(r_readable_lt10)}) is even below 10!
...
(venue_mean_fkscores <- df |>
  group_by(venue) |>
  summarize(m = mean(fkscore), mf = round(m)))
(venue_best  <- venue_mean_fkscores |>
  slice_max(m, n = 1) |>
  glue_data("{venue} is best (mean {mf})"))
(venue_worst <- venue_mean_fkscores |>
  slice_min(m, n = 1) |>
  glue_data("{venue} is worst (mean {mf})"))
@
...
\Plot{boxplots_fkscore}{%
    ...
    \Sexpr{venue_best}, \Sexpr{venue_worst}.
...
\Quote{In this paper, we propose AI-based automation for the completeness checking of privacy policies.} (from \Art{AmaAbuTor22}).
\Quote{This study aims to investigate the urgency and importance of reproducibility and replicability for DL studies on SE tasks.} (from \Art{LiuGaoXia22}).
...
(typl_frac_design <- df |>
  filter(is_design) |>
  mutate(across(fraction_design, ~./100)) |>
  pull(fraction_design) |>
  quantile(c(0.25, 0.75)) |>
  format_percent() |>
  paste(collapse = " to "))

calc_typical_fractions <- function(x) {
  quantile(x / 100, c(0.25, 0.75)) |>
    format_percent() |>
    paste(collapse = "--")
}
...
\Sexpr{typl_frac[typl_frac$is_design == TRUE, "fraction_method"]}
(versus \Sexpr{typl_frac[typl_frac$is_design == FALSE, "fraction_method"]} for empirical works) 
for description of empirical method,
\Sexpr{typl_frac[typl_frac$is_design == TRUE, "fraction_result"]}
(versus \Sexpr{typl_frac[typl_frac$is_design == FALSE, "fraction_result"]})
...
(n_struc <- df |>
  ...
  distinct() |>
  left_join(datasets$ab_structures,
    by = c("_citekey" = "citekey")) |>
  summarize(n_distinct = n_distinct(topicstructure),
    .by = is_design))
@
...
(igap_loc <- raw |>
  filter(icount > 0) |>
  summarize(sum_icount = sum(icount),
    .by = topic) |>
  mutate(r_icount = sum_icount / sum(sum_icount))) 
(igap_in_results <- igap_loc |>
  filter(topic == "result") |>
  pull(r_icount))
(igap_in_method  <- igap_loc |>
  filter(topic == "method") |>
    pull(r_icount))
@
...
<<>>=
icounts <- df |>
  summarize(icount = mean(icount),
    .by = `_citekey`)
 # use icounts instead of df for proper math
(r_igap1 <- sum(df$icount >= 1) / nrow(df))
(r_igap3 <- sum(df$icount >= 3) / nrow(df))
@

More than half of all abstracts (\Sexpr{format_percent(r_igap1)}) have an informativeness gap
(making them improper)
and \Sexpr{format_percent(r_igap3)} have three or more.
...
<<>>=
(r_announce <- sum(df$announcecount >= 1) / nrow(df))
@

Announcements are a waste of space and a nuisance for the reader,
yet \Sexpr{format_percent(r_announce)} of all abstracts have at least one and we consider those improper.
...
<<>>=
ucounts <- df |>
  summarize(ucount = mean(ucount),
    .by = `_citekey`)
# use ucounts instead of df for proper math
(r_ugap <- sum(df$ucount >= 1) / nrow(df))
@

See the rightmost group of Figure~\ref{nonzerofractionbar_xletgroups_missinginfofractions}
for how frequent this is:
About \Sexpr{format_percent(r_ugap)} of all abstracts have one or more understandability gaps.
