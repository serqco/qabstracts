...
(igap_loc <- raw |>
  summarize(
    icount = max(icount), # Need same aggregator as in Python!
    topic = first(topic),
    .by = c(citekey, sidx)
  ) |>
  filter(icount > 0) |>
...
\Todo{Fix formulation after in-Python icount aggregation is decided}
...
yet \Sexpr{format_percent(r_announce)} of all abstracts have at least one and we consider those \emph{improper}.
...
(r_ugap <- sum(ucount_venue$n_ugap) / sum(ucount_venue$n))
...
\Plot{nonzerofractionbar_xletgroups_totalqualityfractions}{%
    ...
    Only \Sexpr{format_percent(r_complete)} of abstracts are \emph{complete}. 
	Only about \Sexpr{format_percent(r_complete_unstruc)} of unstructured abstracts and
  \Sexpr{format_percent(r_complete_design$r_complete)} of design article abstracts are complete.
	...
	Only \Sexpr{format_percent(r_proper)} of abstracts are \emph{proper}.
...
Yet what we find is that only \Sexpr{format_percent(r_proper)} of them indeed are.
...
Only \Sexpr{format_percent(r_complete)} of the investigated software engineering research article abstracts are 
...
Only \Sexpr{format_percent(r_proper)} of the abstracts are \emph{complete}
