...
For example, \Cb{method} is defined as
...
The situation is better for structured abstracts, for which missing \Cb{conclusion}
are still common
...
(i.e., containing all basic elements \Cb{background}, \Cb{objective}, \Cb{method}, \Cb{result}, \Cb{conclusion}).
...
resample_counts <- replicate(
  1000,
  {
    sample_n(design_empir_all, c_design_empir_struct, replace = FALSE) |>
