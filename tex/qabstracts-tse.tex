...
\Todo{add subsection with an evaluation of these two}
...
structured versus unstructured abstracts.  \Todo{perform significance test}
...
\Todo{Argue with lengthy email discussions and number of :ignorediff codes.}
...
\Todo{implement 'complete' and 'proper' (and its parts) in printstats.}

Summing up, a proper abstract should
\Todo{collect the properties}.
Given that we have looked at what are usually considered top-quality venues,
one should expect that a large majority of abstracts fulfills those criteria.
However, only x\% have property1, only x\% have property2, only x\% have property3, [\ldots]. 
And only x\% have all of these properties.
Only x\% are at least complete in the sense that they describe
background, objective, methods, results, and formulate a conclusion.


\subsection{How To: Structured Abstracts are more orderly}\label{structuredgood}

\Todo{ditto for structured abstracts, probably as a side-by-side plot of the partial criteria.}
...
\Todo{spell out}
Only !!!\% are even complete,
only !!!\% also fulfill modest additional criteria: !!!list them!!!
and this despite us analyzing only presumably high quality venues.
...
detail or evidence.
\Todo{When finished, put into .md file in our repo and provide the URL.}
