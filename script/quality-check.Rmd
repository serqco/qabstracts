...
(Note: There is only a difference between `mean` and `max` if we count `>=1` as a gap. If it's everything `>0`, then there is no difference between those two methods.)

A fourth approach would be to not combine the values on abstract-level at all, but just consider all $2\cdot n$ abstract codings separately:
...
(Note: Again, there is only a difference between `mean` and `max` if we count `>=1`, not `>0`, as a gap.)
...
So, what is the 'real' number of abstracts with an i- or u-gap?
Only with the `min`, `mean`, or `max` process can we directly pinpoint which abstracts.
It's a bit harder to wrap your head around the `mixed` numbers.
...
Again, there is another alternative: Consider all $2 \cdot n$ abstract codings individually:
