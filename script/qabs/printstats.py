...
def print_ignorediff_stats(codings: pd.DataFrame, abstracts: pd.DataFrame):
    def remove_shared_codes_from_code_pairs(abstract_ignorediff_codes: pd.DataFrame):
        # Get the counts of codes
        _per_code_count = pd.crosstab(index=abstract_ignorediff_codes['code'], columns="count")
        # Get the codes with only count
        single_codes = _per_code_count[_per_code_count['count'] == 1].index
        # Return the original dataframe wih only the single codes remaining
    ...
    # Remove codes that are in both code pairs. These represent agreement and we only want the disagreement.
    ignorediff_codings = ignorediff_codings.groupby(by='citekey').apply(remove_shared_codes_from_code_pairs)
    # Reset index to original value by taking the second half of the MultiIndex auto-generated by groupby and apply
    ignorediff_codings.index = [original_index for _, original_index in ignorediff_codings.index.to_flat_index()]
